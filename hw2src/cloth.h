...
public:
  ...
  enum IntegrationMethod { Euler, MidPoint, Trapezoid, RungeKutta };

private:

  enum Spring {Structural, Shear, Flexion};
  ...
  void correct(ClothParticle& p1, ClothParticle& p2, double p_correct);
  void provotCorrection(ClothParticle& p, int i, int j);
  std::string stringifyVector(const glm::vec3& vec);
  void updateState(ClothParticle& p, int i_index, int j_index);
  void integrationEuler(ClothParticle& p);
  void integrationMidpoint(ClothParticle& p, int i, int j);
  void integrationRungeKutta(ClothParticle& p, int i, int j);
  void updateForces(ClothParticle& p, int i_index, int j_index);
  glm::vec3 getForces(ClothParticle p, glm::vec3 mid, int i_index, int j_index);
  glm::vec3 calcSpring(Spring s, ClothParticle& pi, int i, int j);
  glm::vec3 calcSpringForce(double k, ClothParticle pi, ClothParticle pj);
  glm::vec3 calcDamping(glm::vec3 vel);

  bool isParticle(int i, int j) {
